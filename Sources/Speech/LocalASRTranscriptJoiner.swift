...
private extension LocalASRTranscriptJoiner {
    static func shouldAttachWithoutSpace(previous: String, next: String) -> Bool {
        ...
        if isCJKSentencePunctuation(last), NoSpaceScript.contains(first) {
            return true
        }
        if NoSpaceScript.contains(last), isOpeningPunctuation(first, in: previous) {
            return true
        }
        return NoSpaceScript.contains(last) && NoSpaceScript.contains(first)
    }
